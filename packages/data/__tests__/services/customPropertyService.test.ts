...
describe("customPropertyService Integration Tests", () => {
  ...
  describe("Custom Property Definitions CRUD", () => {
    ...
    it("should create a property definition successfully", async () => {
      ...
      expect(result.project_ids).toContain(testProjectA.id);
  ...
  describe("Custom Property Values CRUD", () => {
    ...
    it("should set property value successfully (upsert)", async () => {
      ...
      const result = await setPropertyValue({
        task_id: testTaskA.id,
        definition_id: testDefinition.id,
        value: "Option1",
    ...
    it("should update existing property value (upsert)", async () => {
      ...
      const matchingValues = values.filter((val) => val.definition_id === testDefinition.id);
  ...
  describe("Row Level Security (RLS) Tests", () => {
    ...
    it("should not allow users to update definitions in projects they dont own", async () => {
      ...
      await expect(updateDefinition(definition.id, { name: "Hacked!" })).rejects.toThrow();
    });
    ...
    it("should not allow users to set property values on tasks in projects they dont own", async () => {
      ...
      await expect(setPropertyValue(testTaskA.id, definition.id, "Hacked Value")).rejects.toThrow();
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    it("should require authentication for all operations", async () => {
      ...
      await expect(createDefinition(definitionData)).rejects.toThrow("User must be authenticated");

      await expect(setPropertyValue("task-id", "def-id", "value")).rejects.toThrow("User must be authenticated");
    });
  });
